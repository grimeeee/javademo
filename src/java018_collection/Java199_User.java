 ...
 *  하지만 때로는 기존과 같이 모든 객체들을 저장해야 할 때가 있다.
 *  Vector v = new Vector();
 *  v.add(new Integer(20));
 *  v.add(new String("java"));
 *  
 *  2. 와일드 카드의 구성 /Object로 처리됨
 *   <?> : 모든 객체자료형에 대한 배치를 의미한다.
 *   <? super 객체자료형> : 명시된 객체자료형이나 객체자료형의 상위 객체들의 배치를 의미한다.
 *   <? extends 객체자료형> : 명시된 객체자료형이나 객체자료형으로부터 상속받은 하위 객체들의 배치를 의미한다.
 */
...
public class Java199_User {
	...
	public static void openAndAllBox(FruitBox<?> param) { //FruitBox타입에 있는 모든것들을 받을 수 있음
		System.out.println(param.pullout());
		//((Orange)param.pullout()).showYou(); //값이 같음
		((Seoul)param.pullout()).showYou();
	}//end openAndAllBox()
	
	public static void openAndExtendsBox(FruitBox<? extends Fruit> param) { //Fruit이거나 Fruit을 상속받는 것들만 가능
		param.pullout().showYou();
	}//end openAndExtendsBox()
	
	public static void openAndSuperBox(FruitBox<? super Fruit> param) {
		((Seoul)param.pullout()).showYou(); //down-casting
